...
exports.createSuperAdminUser = async (userObject) => {
  try {
    ...
    const tempoLocation = await LocationModel.create({
...
exports.updatePermissions = async () => {
    ...
    PermissionModel.findOne(
      ...
      async (err, obj) => {
        if (obj == null) {
          ...
          console.dir(
            `🟢 Permission Updated Successfully: ${permission.description}`,
            { colors: true }
          );
        }
      }
  ...
  const removed = await PermissionModel.deleteMany({
    code_name: { $nin: codeNames },
  });
...
exports.initialize = async (userObject) => {
  try {
    ...
    console.dir("Initialization Completed successfully", { colors: true });
    process.exit(0);
